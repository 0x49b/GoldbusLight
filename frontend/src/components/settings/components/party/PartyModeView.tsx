...
import {TransferList} from "@/components/scenes/TransferList.tsx";
import type {
    DMXFixture,
...
import {
    ...
    pickLoopbackDevice,
    pickUSBMicDevice
} from "@/lib/dmxPartyAudio.ts";
import {PartyAudioEqualizer} from "@/components/settings/components/party/PartyAudioEqualizer.tsx";
import {
    PartyWLEDDeviceSettings
} from "@/components/settings/components/party/PartyWLEDDeviceSettings.tsx";
...
} from "@/lib/dmxPartyChannelGroups.ts";
                ...
                <CardHeader>
                    <CardTitle>{t("title")}
                        {running && (
                            <>
                                <span
                                    className="text-xs text-muted-foreground">{t("lastFrame", {time: formatPartyTimestamp(party.status.lastFrameAt)})}</span>
                                {mode === "audio" && (
                ...
                <CardContent>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex flex-row gap-3">
                            ...
                            <label
                                className="flex min-w-48 flex-col gap-1 text-xs text-muted-foreground">
                    ...
                    {mode === "audio" && (
                        <div className="space-y-2 rounded-md border bg-muted/30 p-2 mt-3">
                            <div className="flex flex-wrap items-end gap-2">
                                <label
                                    ...
                                    >
                                        ...
                                        <option
                                            value="loopback">{t("audio.presetLoopback")}</option>
                                ...
                                {audioSourcePreset === "custom" && (
                                    <label
                                        className="flex min-w-[15rem] flex-1 flex-col gap-1 text-xs text-muted-foreground">
                                        <span
                                            className="font-medium">{t("audio.inputDevice")}</span>
                                        <select
                                            className="rounded-md border bg-background px-2 py-1 text-sm"
                                            value={config.audioInputDeviceId || ""}
                                            disabled={busy}
